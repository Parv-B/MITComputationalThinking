...
function get_reds(image::AbstractMatrix)
	return broadcast(get_red,image)
end
...
# ╔═╡ 47941b00-b483-44c6-9ff0-0ee167147605
broadcast(value_as_color, get_reds(philip_head))
...
function get_green(color::AbstractRGB)
	return color.g
end

# ╔═╡ eb256680-09ac-4459-93c5-71049c5aba92
function get_greens(image::AbstractMatrix)
	return broadcast(get_green,image)
end

# ╔═╡ 839888e6-4203-463e-a222-2d21ffbb5d1b
function get_blue(color::AbstractRGB)
	return color.b
end

# ╔═╡ 2c0f8163-accf-470a-8879-d3bdbeaf4560
function get_blues(image::AbstractMatrix)
	return broadcast(get_blue,image)
end
...
function mean_color(image)
	...
	mean_r = sum(broadcast(sum, get_reds(image)))/total
	mean_b = sum(broadcast(sum, get_blues(image)))/total
	mean_g = sum(broadcast(sum, get_greens(image)))/total
...
philip_inverted = broadcast(invert, philip_head)
...
# ╔═╡ aaaebc2a-079e-4ede-9eb5-d46afa08d90b
rand()

# ╔═╡ f38b198d-39cf-456f-a841-1ba08f206010
function noisify(x::Number, s)
	n = ((rand()*2)-1)*s
	return clamp(n + x, 0, 1)
end
